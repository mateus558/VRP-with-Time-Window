...
/*void CVRP::shrink_routes(vector<Route*> &solution){
	Point *k = dep.coord;
	
	for(int r = 0; r < solution.size(); r++){
		Route *new_r = new Route;
		vector<pair<Costumer*,double> > savings;
		Costumer *cr = solution[r]->clients[solution[r]->clients.size() - 1];
		for(int p = 0; p < solution.size(); p++){
				Costumer *cp = solution[p]->clients[solution[p]->clients.size() - 1];
				if((cr->arrTime + cr->servTime) < cp->dTime){			
					Point *i = cr->coord; 
					Point *j = cp->coord;
					double saving = calculate_cost(j, k) + calculate_cost(k, i) - calculate_cost(i, j);
					savings.push_back(make_pair(cp, saving));
				}
		}
		sort(savings.begin(), savings.end(), savcmp);
		new_r->clients = solution[r]->clients;
	}
}*/

/*void CVRP::select_candidates(vector<Route*> &solution, vector<vector<Costumer*> > &candidates, vector<double> &times, vector<int> &route_capacity, int &n){
	Point *k = dep.coord;
	...
	for(int r = 0; r < solution.size(); r++){
		...
		vector<pair<Costumer* ,double> > savings;
		Costumer *cr = solution[r]->clients[solution[r]->clients.size() - 1];
		for(int p = 0; p < candidates[r].size(); p++){
			Costumer *cp = candidates[r][p];
			if((cr->arrTime + cr->servTime) < cp->dTime){			
				Point *i = cr->coord; 
				Point *j = cp->coord;
				double saving = calculate_cost(j, k) + calculate_cost(k, i) - calculate_cost(i, j);
		...
		if(!savings.empty()){
			sort(savings.begin(), savings.end(), saving_heuristic);
		
			Costumer* selected = (*(savings.begin())).first;
			if(!visited[selected->id - 1]){
					//Update time
				double waitTime = 0.0;
		
				if(selected->rTime > selected->arrTime){
					waitTime = selected->rTime - selected->arrTime; 	
				}
				//Time = arrivalTime + serviceTime + waitTime
				times[r] = selected->arrTime + selected->servTime + waitTime;
				//Add the client to the solution and set as visited
				solution[r]->clients.push_back(selected);
				visited[selected->id - 1] = true;
				n++;
			}
		}
